...
makeConsistent <- function(
  ...
  if( !is.null(sampledata)){
    ...
    if(!is.null(fasta)){
      ...
      if( fasta.file != ""){
        message("makeConsistent: new fasta will be written to ", fasta.file)
        if(write){
          write.table(clean.fasta, fasta.file, quote=F, sep="", col.names=F)
          out$fasta.file <- fasta.file
          message("makeConsistent: new fasta written to ", fasta.file)
        }
