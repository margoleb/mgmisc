...
preparePICRUSt2 <- function(
  ...
  if( picrust.version != 0 ){
    stop("preparePICRUSt2: ", picrust, " is not a valid executable PICRUSt2 pipeline script" )
  }
  message("preparePICRUSt2: now running PICRUSt2 pipline with ", processors, " cores")
  ...
  result <- readPICRUSt2Dir(dir=picrust.outdir)
  if(plot.nsti){
