...
splitFastqAmpliconsByTaxonomy <- function( # for amplicons too long to be split by primers
  ...
  for(n in 1:length(amplicon.names)){
    ...
    res <- as.data.frame(dada2::assignTaxonomy(seqs=seqs, refFasta=db,  minBoot = minBoot, multithread=multithread))
    rownames(res) <- seq(1:length(res))
    reads.to.select <- rownames(res[res[[level]] == tname])

    if(length(reads.to.select) > 0){
      ...
      # write filtered fastq files
      ShortRead::writeFastq(outr1_trimmed, r1_filename, mode=mode)
      ShortRead::writeFastq(outr2_trimmed, r2_filename, mode=mode)
