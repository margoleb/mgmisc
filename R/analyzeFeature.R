...
analyzeFeature <- function(
  ...
  if(!is.null(experiment)){
    ...
    directory <- paste0(experiment$working.dir, "/", featurename)
    ...
    if(analysis == 'alpha-diversity'){
      if(!is.null(setRarefied)){
        if(is.null(facetting.variables)){
          ...
          if(is.null(comps)){
            com <- combn(levels(as.factor(setRarefied[[variable]])), 2)
          ...
          fname <- paste0(basename, "_alpha.diversity_", variable, ".facetting_", paste(facetting.variables, collapse="_"), ".svg")
            ...
            if(is.null(result$facets)){
              if(featurename == "OTUs"){
                result[[d]]$facets <- facets
                result[[d]]$alpha.diversity <- list()
                result[[d]]$alpha.diversity$plot <- p
              }else{
              ...
              for(f in names(facets)){
                if(featurename == "OTUs"){
                  result[[d]]$facets[[f]]$alpha.diversity <- facets[[f]]$alpha.diversity
                }else{
...
#          assign(dbg, facets, envir=parent.frame())
          ...
          if(!is.null(result$facets)){
            for(f in names(facets)){
              if(featurename == "OTUs"){
                result[[d]]$facets[[f]]$beta.diversity <- facets[[f]]$beta.diversity
              }else{
          ...
          if(featurename == "OTUs"){
            result[[d]]$beta.diversity <- list()
            result[[d]]$beta.diversity$plots <- plots
          }else{
      ...
      if(featurename %in% c("ASVs", "OTUs")){
        ...
        if(is.null(facetting.variables)){
          ...
          if(!is.null(result$facets)){
            for(f in names(facets)){
              if(featurename == "OTUs"){
                result[[d]]$facets[[f]]$taxonomy <- facets[[f]]$taxonomy
              }else{
      ...
      if(is.null(facetting.variables)){
        ...
        if(featurename == "OTUs"){
          result[[d]]$diff.features <- list()
          result[[d]]$diff.features$ASVs <- res$deseq
        }else{
          result$diff.features <- list()
          result$diff.features$ASVs <- res$deseq
        }
        if(!is.null(result$taxonomy)){
          ...
          for(tlevel in names(result$taxonomy$taxtabs)){
            ...
            if(featurname == "OTUs"){
              result[[d]]$diff.features[[tlevel]] <- res$deseq
            }else{
              result$diff.features[[tlevel]] <- res$deseq
            }
          }
          if(featurename == "OTUs"){
            result[[d]]$diff.features$final.table <- finaltable
          }else{
        ...
        if(!is.null(result$facets)){
          for(f in names(facets)){
            if(featurename == "OTUs"){
              result[[d]]$facets[[f]]$diff.features <- facets[[f]]$diff.features
            }else{
