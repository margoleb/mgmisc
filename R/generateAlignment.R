  ...
  if(reference.alignment != ""){
    ...
    if(!inherits(s, 'try-error')){
      ...
      if(is.null(start) | is.null(stop)){
        message("    generateDistMat: determining start and stop positions")
        ...
        message(paste0("    generateDistMat: alignment will be trimmed to ", start, " - ", stop, " position"))
        outlist$start <- start
        outlist$stop <- stop
        outlist$count.file <- count.file
      }
      # screening for sequences covering the desired region of the alignment
      message("    generateDistMat: trimming alignment")
      ...
      message("    generateDistMat: filtering alignment")
