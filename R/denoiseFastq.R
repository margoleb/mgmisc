...
#' @param filtered A logical indicating if precomputed filtering results should be used. If TRUE there needs to be a 'filtered' directory i the dir where fastq files are stored and it must contain filtered fastq files. Defaults to FALSE.
...
denoiseFastQSet <- function(directory=".",
  ...
  if(!filtered){
    ...
    out <- dada2::filterAndTrim(fnFs, filtFs, fnRs, filtRs, maxN=0, maxEE=c(2,2), truncLen=truncLen, truncQ=2, rm.phix=TRUE, compress=TRUE, multithread=multithread)
  ...
  track <- cbind(out, report, rowSums(seqtab), rowSums(seqtab.nochim))
...
process_together <- function(
  ...
  if(reads == 'both' | reads == 'reverse'){
    ...
    dadaRs <- dada2::dada(derepRs, err=errR, multithread=multhread, pool=pool)
  }
  if(reads == 'both'){
