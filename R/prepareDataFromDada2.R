...
prepareDataFromDada2 <- function(
    ...
    basename = "bacteria",
    rand.seed = 667,
    fasta.file = NULL, # name of a fasta file with ASVs sequences, ALWAYS NOT ALIGNED
    count.file = NULL, # name of a count file with ASVs number per sample
    OTU = FALSE,
...

{
  ...
  if(is.character(design)){
    design <- read.table(design, header=T, sep=design.sep)
  }
  ...
  outlist$metadata$working.dir <- getwd()
  outlist$metadata$classification.db <- classification.db
  ...
  out <- mgmisc1::makeConsistent(featuretab=outlist$ASVs$featuretab,
                                 ...
                                 fasta.file=outlist$original.data$fasta.file,
  ...
  outlist$metadata$alignment_trimming <- list(start, stop)
  names(outlist$metadata$alignment_trimming) <- c("start", "stop")
  ...
  if( generate.tree ){
    ...
    fasta.file <- sub("\\.fasta", ".clean.fasta", f$fasta.file) # aligned at this point
    message(paste0("prepareDataFromDada2: renaming ", t$fasta.file, " to ", fasta.file))
    system2("mv", paste0(t$fasta.file, " ", fasta.file))
    ...
    out <- mgmisc1::makeConsistent(featuretab=outlist$ASVs$featuretab,
                                   ...
                                   fasta=fasta, fasta.file=fasta.file,
    ...
    outlist$ASVs$featureannot <- out$featureannot
    outlist$ASVs$fasta <- out$fasta
    outlist$ASVs$fasta.file <- fasta.file
    outlist$ASVs$aligned <- TRUE
    ...
    if(!is.null(tree)){
      ...
      out <- mgmisc1::makeConsistent(featuretab=outlist$ASVs$featuretab,
                                     ...
                                     fasta=outlist$ASVs$fasta, fasta.file=outlist$ASVs$fasta.file,
  ...
  if(as.logical(rarefy)){
    ...
    outlist$ASVs$rarefied.data <- mgmisc1::rarefyData(featuretab=outlist$ASVs$featuretab, featureannot=outlist$ASVs$featureannot, sampledata=outlist$ASVs$sampledata, tree=outlist$ASVs$tree, tree.file=tree.file, fasta=f$fasta, fasta.file=fasta.file, count_table=f$count_table, count.file=count.file, rarefy=rarefy, depth=depth)
  }
  makeOTUs = FALSE
  if(length(OTU) == 1){
    if(is.logical(OTU)){
  ...
  if(makeOTUs){
      for( diss in OTU ){
        if( !is.na(diss) & diss < 1 ){
          ...
          start <- outlist$metadata$alignment_trimming$start
          stop <- outlist$metadata$alignment_trimming$stop
          ...
          otus <- constructOTUs( fasta.file=fasta.file, count.file=count.file, diss=c(diss), aligned=aligned, pairwise.alignment=pairwise.alignment, reference.alignment=reference.alignment, start=start, stop=stop, classification.db=classification.db, processors=processors)
          ...
          if(remove.rare){
            ...
            outlist$OTUs$dists[[diss]]$featuretab <- otutab.final
            outlist$OTUs$dists[[diss]]$tree <- tree
          }
          assign("cos", otus, envir=parent.frame())

          if(rarefy){
            message(paste0("Rarefying OTUs to ", depth, " reads ", rarefy, " times"))
            tree.file <- sub("\\.tre", ".rarefied.tre", tree.file)
            out <- rarefyData(featuretab=outlist$OTUs$dists[[diss]]$featuretab, sampledata=outlist$original.data$sampledata, featureannot=outlist$OTUs$dists[[diss]]$featureannot, tree=tree, tree.file=tree.file, fasta=NULL, fasta.file=NULL, count_table=NULL, count.file=NULL, depth=depth, rarefy=rarefy)
